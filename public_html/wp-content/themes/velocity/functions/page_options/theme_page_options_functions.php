...
function velocity_add_custom_scripts() {
...
add_action('admin_head','velocity_add_custom_scripts');
				...
				switch($field['type']) {
					...
					case 'slider_list':
						...
						if(shortcode_exists("showbiz")){
							...
							$showbiz_sliders = $slider->getArrSliders();
							
							echo "<option value=''>--- Show Biz Sliders ---</option>";
							foreach ( $showbiz_sliders as $showbiz_slider ) 
							{
								$alias = $showbiz_slider->getAlias();
								$title = $showbiz_slider->getTitle();
								$checked="";
		            		 	if('showbiz_'.$alias==$meta) $checked="selected";
		            		 	echo "<option value='showbiz_$alias' $checked>".$title."</option>";
							}
			            }
	                	echo '</select>';
					break;
							...
							foreach ($tp_home_teasers as $teaser) {
								...
								if(in_array($teaser, $tp_showbiz_teasers) || $teaser_headline_short ==""){
									...
									if(strlen($teaser_headline_short)>14)
										...
										$teaser_list_used .= '<li class="widget ui-draggable"><input name="velocity_home_teasers[]" type="hidden" value="'.$teaser.'">'.$teaser_headline_short.'</li>
									';
								}
							}
						}
						...
						if(is_array($tp_showbiz_teasers)){
							foreach ($tp_showbiz_teasers as $teaser) {
								...
								if(!is_array($tp_home_teasers) || !in_array($teaser, $tp_home_teasers))
									$teaser_list_unused .= '<li class="widget ui-draggable"><input name="velocity_home_teasers[]" type="hidden" value="'.$teaser.'" disabled>'.$teaser_headline_short.'</li>
								';
							}
						}

						if (!strpos($teaser_list_used, 'value="velocity_home_content">Page Content</li>'))
						$teaser_list_unused .= '<li class="widget ui-draggable"><input name="velocity_home_teasers[]" type="hidden" value="velocity_home_content" disabled>Page Content</li>';	
...
function save_custom_meta($post_id) {
    ...
    if(isset($_POST['post_type']) && !defined('DOING_AJAX')){
	    // which fields to use
