...
$tabs_nav = '';
$tabs_nav .= '<ul class="wpb_tabs_nav ui-tabs-nav velocitytab clearfix">';
foreach ( $tab_titles as $tab ) {
    preg_match('/title="([^\"]+)"(\stab_id\=\"([^\"]+)\"){0,1}/i', $tab[0], $tab_matches, PREG_OFFSET_CAPTURE );
    if(isset($tab_matches[1][0])) {
        $tabs_nav .= '<li><a href="#atab-'. (isset($tab_matches[3][0]) ? $tab_matches[3][0] : str_replace("%","_",sanitize_title( $tab_matches[1][0] ) ) ) .'">' . $tab_matches[1][0] . '</a></li>';
